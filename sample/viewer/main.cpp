  ...
  while (capture->isRun()) {
    ...
    *capture >> lasers;
    ...
    for (const velodyne::Laser& laser : lasers) {
      ...
      float x = static_cast<float>((distance * std::cos(vertical)) *
                                   std::sin(azimuth));
      float y = static_cast<float>((distance * std::cos(vertical)) *
                                   std::cos(azimuth));
...
int main(int argc, char* argv[]) {
  ...
  viewer.registerKeyboardCallback(
      [](const cv::viz::KeyboardEvent& event, void* cookie) {
        // Close Viewer
        if (event.code == 'q' &&
            event.action == cv::viz::KeyboardEvent::Action::KEY_DOWN) {
